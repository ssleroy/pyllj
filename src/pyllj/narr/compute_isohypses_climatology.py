...
def compute_isohypses_climatology( yearrange:str, dataroot:str=default_dataroot ): 
    ...
    generated compute_narr_isohypses. 
    ...
    for year in range( yearrange[0], yearrange[1]+1 ): 
        for imonth in range(12): 
            ...
            if year == yearrange[0] and imonth == 0: 
                ...
                metadatavars = [ 'x', 'y', 'lon', 'lat', 'level', 'month', 'hour', 'Lambert_Conformal' ]
                ...
                for var in [ 'uwnd', 'vwnd' ]: 
                    dv = d.variables[var]
                    ov = o.createVariable( var, dv.dtype, dimensions=('month','hour','level','y','x') )
                    atts = { att: dv.getncattr( att ) for att in dv.ncattrs() }
                    ov.setncatts( atts )

                #  Local variables. 

                uwnd = np.ma.zeros( (nmonths,nhours,nlevels,ny,nx), dtype=np.float32 )
                vwnd = np.ma.zeros( (nmonths,nhours,nlevels,ny,nx), dtype=np.float32 )
    ...
    o.setncatts( { 
        'file_type': "narr_isohypsic_analysis_climatology", 
        'yearrange': [ np.int32(yearrange[0]), np.int32(yearrange[1]) ], 
        'author': "Stephen Leroy (stephen.leroy@janusresearch.us", 
        'create_time': datetime.now( tz=timezone.utc ).strftime( "%d %b %Y %H:%M:%S UTC" ) } )
...
def main(): 
    ...
    parser = argparse.ArgumentParser( description="Generate an annual cycle, diurnal " + \
            ...
            "based on the output of compute_narr_isohypses" )
