...
from .libnarr import cycle_time, default_dataroot
...
def compute_climatology( yearrange:{tuple,list}, dataroot:str=default_dataroot ): 
    ...
    based on the output of compute_narr_watervaporflux. 
    ...
    for imonth in range(12): 
        for year in range(yearrange[0],yearrange[1]+1): 
            ...
            if out is None: 
                ...
                for variable_name in [ "Lambert_Conformal", "lon", "lat" ]: 
                ...
                for i in range(12): 
                    out['column_climatology'].append( 
                            { key: np.zeros( (nzulu,ny,nx), dtype=np.float32 ) for key in variables } | \
                            { key+"_var": np.zeros( (nzulu,ny,nx), dtype=np.float32 ) for key in variables } )
            ...
            for vert in [ 'column', 'pbl' ]: 
                ...
                for key in variables:  
                    clim[key] += d.groups[vert].variables[key][:].reshape( nzulu, ndays, ny, nx ).mean(axis=1)
                    clim[key+"_var"] += ( d.groups[vert].variables[key][:].reshape( nzulu, ndays, ny, nx ).mean(axis=1) )**2 
    ...
    outputpath = os.path.join( output_root, "watervaporflux.nc" )
    ...
    for vert in [ 'column', 'pbl' ]: 
        ...
        for varname in variables: 
            varname1 = varname + ""
            v = g.createVariable( varname1, 'f', dimensions=("month","zulu","y","x") )
...
def main(): 
    ...
    parser = argparse.ArgumentParser( prog="compute_narr_climatology", description=
            ...
            DATAROOT/NARR/watervaporflux/watervaporflux.nc. DATAROOT is defined using the 
            --dataroot option.""" )
    ...
    parser.add_argument( "yearrange", type=str, help="""Inclusive range of years over which 
            ...
            watervaporflux files produced by compute_narr_watervaporflux. The 
            argument must have format "YYYY:YYYY".""" )

    parser.add_argument( "--dataroot", dest="dataroot", type=str, default=default_dataroot, 
            help='The root of NARR processing for low-level jet research; the default is ' + \
            f'"{default_dataroot}".' )
