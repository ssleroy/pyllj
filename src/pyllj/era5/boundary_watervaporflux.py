...
from .libera5 import RetClass, cycle_time, epoch, output_time_units
from ..libutils import GridInterpolator 
from ..parameters import boundaries, boundarynames, default_dataroot, Rearth

watervaporflux_subdir = "ERA5/watervaporflux"
output_subdir = "ERA5/boundaryflux"
...
def boundary_watervaporflux( boundary, daterange:{tuple,list}, dataroot=default_dataroot ): 
    ...
    for interval in range( slons.size - 1 ): 
        ds = 2 * Rearth * np.arcsin( np.linalg.norm( pvs[interval+1,:] - pvs[interval,:] ) )
    ...
    v.setncatts( { 'description': "The time corresponding to the data (UTC)", 
                  'units': output_time_units + " since " + epoch.strftime( "%Y-%m-%d %H:%M:%S" ) } )
    ...
    d.setncatts( {
            'file_type': "ERA5_boundary_watervaporflux", 
            'description': "This file contains the flux of total column water vapor across a " + \
                ...
                "the ECMWF Reanalysis version 5 (ERA5).", 
    ...
    d.variables['time'][:] = [ np.int32( ( t - epoch ) / timedelta( **{ output_time_units: 1 } ) + 0.001 ) for t in times ]
...
def main(): 
    ...
    parser = argparse.ArgumentParser( prog="compute_era5_boundary_watervaporflux", 
            description="""Compute column-integrated water vapor and the component 
            column-integrated water flux over the entire domain of the ECMWF 
            Reanalysis version 5 (ERA5). Column-integrated water vapor is also 
