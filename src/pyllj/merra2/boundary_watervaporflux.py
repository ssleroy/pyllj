...
def boundary_watervaporflux( boundary, daterange:{tuple,list}, dataroot=default_dataroot ): 
    ...
    for itime in tqdm( range(ntimes), desc="  Time" ): 
        ...
        for groupname, g in d.groups.items():
            ...
            try: 
                ...
                ret.update( success=False, messages="UnreadableFile", comments=f'Unable to read from file {file}' )
                d.close()
                return ret
    ...
    d.setncatts( {
            'file_type': "MERRA2_boundary_watervaporflux", 
            'description': "This file contains the flux of total column water vapor across a " + \
                ...
                "the NASA Modern Era Retrospective Reanalysis version 2 (MERRA2).", 
...
def main(): 
    ...
    parser = argparse.ArgumentParser( prog="compute_merra2_boundarywatervaporflux", 
            description="""Compute column-integrated water vapor and the component 
            column-integrated water flux over the entire domain of the Modern Era 
            Retrospective Reanalysis version 2 (MERRA2). Column-integrated water 
            vapor is also included in the output.""" )
